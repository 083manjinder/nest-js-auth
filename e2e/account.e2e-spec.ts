...
describe('Account', () => {
  ...
  it('/GET account', async () => {
    await request(app.getHttpServer()).get('/api/account').expect(200);
  });
  ...
  it('/POST account update settings', async () => {
    ...
    await request(app.getHttpServer()).post('/api/account').send(savedTestUser).expect(200);
  ...
  it('/POST change password', async () => {
    await request(app.getHttpServer()).post('/api/account/change-password').send(testPasswordChange).expect(200);
